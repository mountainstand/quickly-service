...
//                let cert = certificateString  //Assign your certificate string to cert
//                let tmpData = Data(base64Encoded: cert)!
//                let tmpData = cert.data(using: .utf8)!
                
                // let pemComponents = certificateString.components(separatedBy: .newlines)
                // let base64String = pemComponents.filter { !$0.hasPrefix("-----") }.joined()
                // let tmpData = Data(base64Encoded: base64String)!
                
                // let certificate = SecCertificateCreateWithData(nil, tmpData as CFData)!
                // let certificateData = SecCertificateCopyData(certificate) as Data
                
                // p.identityData = certificateData
                debugPrint("Before CertData")
                if let certData = self.convertCertificateStringToData(certificateString) {
                    debugPrint("Got CertData", certData)
                    let certDataKey = "vpn_\(type)_certData"
                    self.kcs.storeCertificateInKeychain(certData: certData, key: certDataKey)
                    p.identityData = self.kcs.retrieveCertificateFromKeychain(key: certDataKey)
                }
            ...
            self.vpnManager.saveToPreferences(completionHandler: { (error) -> Void in
                debugPrint("inside saveToPreferences")
                guard error == nil else {
                    let msg = "VPN Preferences error: \(error!.localizedDescription)"
                    debugPrint(msg)
                    VPNStateHandler.updateState(FlutterVpnState.error.rawValue, errorMessage: msg)
                ...
                self.vpnManager.loadFromPreferences(completionHandler: { error in
                    debugPrint("inside loadFromPreferences")
                    guard error == nil else {
                        let msg = "VPN Preferences error: \(error!.localizedDescription)"
                        debugPrint(msg)
                        VPNStateHandler.updateState(FlutterVpnState.error.rawValue, errorMessage: msg)
    ...
    func convertCertificateStringToData(_ certString: String) -> Data? {
        let trimmedCert = certString
            .replacingOccurrences(of: "-----BEGIN CERTIFICATE-----", with: "")
            .replacingOccurrences(of: "-----END CERTIFICATE-----", with: "")
            .replacingOccurrences(of: "\n", with: "") // Remove extra newlines/spaces
        
        return Data(base64Encoded: trimmedCert)
    }

    func startTunnel() {
        do {
            try self.vpnManager.connection.startVPNTunnel()
            debugPrint("started tunnel")
