...
// For PEM certificate Installation
...
public class VpnStateService extends Service {
    ...
    public void connect(Bundle profileInfo, boolean fromScratch) {
        ...
        if (profileInfo.containsKey("CertificateString")) {
            installPemCertificate(context, profileInfo.getString("CertificateString"));
        }
    ...
    /**
     * Installs a PEM certificate into the Android KeyStore.
     *
     * @param context   The application context (not used in this function but kept for future extensions).
     * @param pemString The certificate string in PEM format.
     * @return true if the certificate was successfully installed, false otherwise.
     */
    public static boolean installPemCertificate(Context context, String pemString) {
        try {
            // Convert the PEM string into an InputStream
            ByteArrayInputStream inputStream = new ByteArrayInputStream(pemString.getBytes(StandardCharsets.UTF_8));
            ...
            // Add the certificate to the KeyStore under the alias "vpnCaCert"
            keyStore.setCertificateEntry("vpnCaCert", certificate);

            // Initialize a TrustManagerFactory with the populated KeyStore
            TrustManagerFactory trustManagerFactory = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm());
            trustManagerFactory.init(keyStore);

            // Retrieve the TrustManagers
            ...
            // Check if the TrustManager is correctly initialized
            if (trustManager != null) {
                Log.d("VPN", "Certificate installed successfully!");
                return true;
            } else {
                Log.e("VPN", "Failed to install certificate!");
                return false;
            }
